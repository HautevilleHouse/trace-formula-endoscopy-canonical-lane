...
In-paper anchor: `paper/TRACE_FORMULA_ENDOSCOPY_PREPRINT.md` (bridge and final strict margin).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of stable orbital transfer and endoscopic matching across admissible trace-formula packets through a multi-lane endoscopic super-architecture`.
...
2. the endpoint-transfer lock `endoscopic_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_TFE` remains positive.
...
Payload: verify `endoscopic_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_TFE` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `endoscopic_transfer`
