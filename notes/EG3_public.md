...
In-paper anchor: `paper/TRACE_FORMULA_ENDOSCOPY_PREPRINT.md` (`TFE_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of stable orbital transfer and endoscopic matching across admissible trace-formula packets through a multi-lane endoscopic super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`TFE_G3` closes when `kappa_compact` provides the compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_TFE`.
...
If near-failure extraction and no-Zeno spacing hold, then `TFE_G3` prevents hidden drift before rigidity is applied.
